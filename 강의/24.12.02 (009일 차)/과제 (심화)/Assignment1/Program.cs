    ...
    internal class Program
    {
        ...
        private static void ChangeWeapon(ref Soilder soilder)
        {
            ...
            while (true)
            {
                ...
                int.TryParse(Console.ReadLine(), out weaponNumber);

                // 1~3이 아닐 경우,
                if (weaponNumber < 1 || soilder.weapons.Length < weaponNumber)
                {
                ...
                if (weaponNumber == soilder.currentWeapon)
                {
            ...
            soilder.currentWeapon = weaponNumber;

            // 본인 손에 들고 있는 무기와는 다른 무기를 정상적으로 골랐다면 콘솔 출력을 통해 해당 무기로 바뀌었음을 출력합니다.
            Console.WriteLine();
            Console.Write($"무기가 {soilder.weapons[weaponNumber - 1].name}로 바뀌었습니다.");
            Console.WriteLine();
        }
